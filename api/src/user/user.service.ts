...
export class UserService {
    ...
    public async login(userLogin: UserLogin): Promise<UserResponse> {
        ...
        const token = sign({ email: user.email, id: user.id }, this.jwtSecretKey, { expiresIn: "10h" });
    ...
    public async registration(user: UserRegistration): Promise<UserResponse> {
        try {
            ...
            await newUser.save();
            const token = sign({ email: newUser.email, id: newUser.id }, this.jwtSecretKey, { expiresIn: "10h" });
            return new UserResponse({ token });
        } catch(error) {
