...
export class DiagramService {
    public constructor(private calculateProfileService: CalculateProfileService) {}
    ...
    public create(nameIndex: string, profile: IIndex[]): DiagramProfile[] {
        ...
        for (const coefficient of index.coefficients) {
            ...
            if (coefficient.metric !== null) {
                ...
                result = {
                    nameIndex: `${coefficient.name} (${coefficient.value})`,
                    value: this.calculateProfileService.calculate(coefficient.nameConnect, profile)
                }
            }
            diagram.push(result);
        }
